...
try
{
    ...
    switch (typedOption)
    {
        case 1:
            ...
            break;
        case 2:
            //First element from TypedNumbers list for the Subtracting Method:

            var firstElementFromTypedNumbers = typedNumbers[0];

            for (int index = 1; index < typedNumbers.Count(); index++)
            {
                firstElementFromTypedNumbers = firstElementFromTypedNumbers - typedNumbers[index];
                result = firstElementFromTypedNumbers;
            }
